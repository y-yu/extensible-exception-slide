...
\begin{frame}[fragile]
  ...
  \uncover<4->{
    \begin{exampleblock}{例}
      DbExceptionからRootExceptionへの変換ができることを表す
...
\begin{frame}[fragile]
  ...
  \begin{center}
    \uncover<4->{
       \begin{tikzpicture}
         \calloutquote[width=4cm,position={(-1.2,-0.2)},fill=green!30,rounded corners]{これで動くの？}
       \end{tikzpicture}
    }
  \end{center}
...
\begin{frame}[fragile]
  ...
  %\uncover<4->{
  %  \lstinputlisting[style=scala, fi]
  %}
\end{frame}
